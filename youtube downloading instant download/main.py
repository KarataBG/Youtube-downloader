...
# Оправи age restrict или поне да показва че е това проблема
# Оправи от видео идва заглавие след това при плейлист остава заглавието
# Имената от плейлиста не се пренасят на нов ред
...
class Panel:
    ...
    def definePlaylist(self):
        ...
        splitedString = ""
        n  = 32
        for index in range(0, len(self.playlist.title), n):
            splitedString+=(self.playlist.title[index : index + n]) + "\n"
    ...
    def downloadPlaylist(self, audioOnly: bool, maxQuality: bool, videoQuality: str, mime_type: str,
        ...
        if self.isDownloadingPlaylist:
            print("В момента се тегли от плейлист")
            return
        self.isDownloadingPlaylist = True
        ...
        for vid in self.playlist.videos:
            ...
            while True:
                if self.threadNumber < queue:
                    self.threadNumber += 1
                    t = threading(target=self.downloadVideo,
    ...
    def downloadVideo(self, audioOnly: bool, maxQuality: bool, videoQuality: str, mime_type: str, audioQuality: str,
                 url: YouTube, title: str):
        print(self.directory +"/"+ url.title)
        ...
        if audioOnly:
            ...
            try:
                moviepy.video.io.ffmpeg_tools.ffmpeg_extract_audio(audioFile, audioOutput, int(audio.abr[:-4]),48000)
